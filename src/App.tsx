...
export default function App() {
  ...
  return (
    <div style={{ fontFamily: "system-ui,Segoe UI,Roboto,Arial", padding: 20, maxWidth: 1000, margin: "0 auto" }}>
      <h1>She react on my vite till i localhost</h1>
      <p style={{ color: "#444" }}>
        phoenix
      </p>
      ...
      <div style={{ marginBottom: 12 }}>
        ...
        {status === "indexing" && (
          <span>
            {" "}
            — downloaded {progress.done}/{progress.total} ({Math.round((progress.done / Math.max(1, progress.total)) * 100)}%)
          </span>
        )}
        {status === "ready" && <span> — indexed {progress.total} documents</span>}
      </div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
        ...
        />
        <button onClick={() => doSearch(query)} disabled={!query || status === "indexing" && progress.done === 0}>
          Search
        </button>
        <button
          onClick={() => {
            // quick re-run with current query (useful as index grows)
            doSearch(query);
          }}
        >
          Refresh results
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
        <div style={{ color: "#666" }}>
          {results.length} result{results.length !== 1 ? "s" : ""} {results.length > 0 && <> — page {page} / {totalPages}</>}
        </div>
      ...
      <div style={{ display: "flex", gap: 8, justifyContent: "center", marginTop: 16 }}>
        ...
        <div style={{ padding: "6px 10px", border: "1px solid #eee", borderRadius: 6, minWidth: 120, textAlign: "center" }}>
